  ...
  home-manager.users.${username} = {
    programs.nvf.settings.vim = {
      ...
      autocmds = [
        {
          event = ["LspAttach"];
          desc = "Disable semantic highlighting for strings only";
      ...
      in {
        ${name} = {
          ...
          package = pkgs.vimUtils.buildVimPlugin {
            inherit name;
            src = pkgs.writeTextDir "lua/${name}/init.lua" ''
              local templates = {
                	rust = {
                		string = {
                			langs = {
                ...
                local function write(lang, file, content)
                  ...
                  if vim.fn.isdirectory(queries_path) == 0 then
                    ...
                    vim.notify("Failed to write to " .. lang_path .. "/" .. file .. ".scm", vim.log.levels.ERROR)
                  end
                end
